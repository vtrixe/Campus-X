...
const formSchema = z.object({
  ...
  domain: z.string().min(1, {
    message: "Server domain is required."
  })
});

export const EditServerAdmin = () => {
  const { isOpen, onClose, type, data } = useDialog();
  const router = useRouter();

  const isModalOpen = isOpen && type === "editServerAdmin";
  const { server } = data;
      ...
      <DialogContent className="bg-white text-black p-0 overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center text-zinc-500">
            Give your server a personality with a name, domain, and an image. You can always change it later.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
